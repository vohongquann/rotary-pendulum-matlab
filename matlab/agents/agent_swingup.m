function agent = agent_swingup(env)
% myAgentBalance - Cấu hình SAC agent cho nhiệm vụ giữ thăng bằng
    ...
    actorLG = addLayers(actorLG, fullyConnectedLayer(8, "Name", "fc1"));
    actorLG = addLayers(actorLG, reluLayer("Name", "relu1"));
    actorLG = addLayers(actorLG, fullyConnectedLayer(8, "Name", "fc2"));
    ...
    % Kết nối mean branch
    actorLG = connectLayers(actorLG, "relu2", "meanOut");
    % Kết nối std branch
    actorLG = connectLayers(actorLG, "relu2", "fcLogStd");
    ...
    agentOpts.DiscountFactor = 0.9999;
    agentOpts.TargetSmoothFactor = 1e-3;
    agentOpts.ExperienceBufferLength = 1e8;
    agentOpts.MiniBatchSize = 2048;
    agentOpts.NumWarmStartSteps = 1000;
    agentOpts.ActorOptimizerOptions = rlOptimizerOptions("LearnRate",5e-3);
    agentOpts.CriticOptimizerOptions = rlOptimizerOptions("LearnRate",6e-3);
