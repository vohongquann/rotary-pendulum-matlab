function startup()
    proj = matlab.project.rootProject;
    
    % Thêm các thư mục con cần thiết vào path
    foldersToAdd = {
        fullfile(proj.RootFolder, 'cad')
        fullfile(proj.RootFolder, 'scripts')
        fullfile(proj.RootFolder, 'results')
        fullfile(proj.RootFolder, 'config')
        fullfile(proj.RootFolder, 'agents')
        fullfile(proj.RootFolder, 'environment')
        fullfile(proj.RootFolder, 'trained_models')
        fullfile(proj.RootFolder, 'simulink')
    };

    for i = 1:numel(foldersToAdd)
        addpath(genpath(foldersToAdd{i}));
    end
    
    disp('[InvertedPendulumProject] Project initialized!');
end
